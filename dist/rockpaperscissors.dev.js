...
var humanScore = 0; // Human player's score

var computerScore = 0; // Computer player's score

var roundsPlayed = 0; // Number of rounds played

var totalRounds = 5; // Total number of rounds in the game
...
function updateDOM(result, humanChoice, computerChoice) {
  var resultElement = document.getElementById("result"); // Get the result element
  ...
  if (result === "human") {
    ...
    resultElement.textContent = "You win this round! ".concat(humanChoice, " beats ").concat(computerChoice, "."); // Display win message
  } else if (result === "computer") {
    computerScore++; // Increment computer score

    resultElement.textContent = "You lose this round! ".concat(computerChoice, " beats ").concat(humanChoice, "."); // Display lose message
  } else {
    resultElement.textContent = "It's a tie this round! Both chose ".concat(humanChoice, "."); // Display tie message
  } // Update the score display


  scoreElement.textContent = "Score - You: ".concat(humanScore, ", Computer: ").concat(computerScore); // Update score text
} // Function to declare the final winner after all rounds are played


function declareWinner() {
  var finalResultElement = document.getElementById("finalResult"); // Get the final result element
...
function makeChoice(humanChoice) {
  var computerChoice = getComputerChoice(); // Get computer's choice

  var result = playRound(humanChoice, computerChoice); // Play a round and get the result
...
document.getElementById("rock").addEventListener("click", function () {
  return makeChoice("rock");
}); // Play a round with "rock" choice

document.getElementById("paper").addEventListener("click", function () {
  return makeChoice("paper");
}); // Play a round with "paper" choice

document.getElementById("scissors").addEventListener("click", function () {
  return makeChoice("scissors");
}); // Play a round with "scissors" choice
